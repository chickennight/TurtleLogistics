...
#define SUB_TOPIC_02 "/div/servo3/info"
#define SUB_TOPIC_03 "/mod/div/servo3/angle"
#define SUB_TOPIC_04 "/mod/div/servo3/servo_interval"
#define SUB_TOPIC_05 "/mod/div/servo3/ir_interval"
...
void setup() {
  ...
  Div_Servo.publish(PUB_TOPIC_00, "{\"dev\":\"Div_Servo3\",\"content\":\"AWS Connect Success\"}");
...
void Subscribe_callback(char *topic, byte *payload, unsigned int length){
  ...
  if(strcmp(SUB_TOPIC_02, topic)==0){      //  /div/servo1/info
    ...
    angle = (int)doc["angle"];
    Div_Servo.publish(PUB_TOPIC_00, "{\"dev\":\"Div_Servo3\",\"content\":\"Angle Changed\"}");
    
  }
  else if(strcmp(SUB_TOPIC_04, topic)==0){      //  /mod/div/ser1/servo_interval
    servo_interval = (int)doc["servo_interval"];
    Div_Servo.publish(PUB_TOPIC_00, "{\"dev\":\"Div_Servo3\",\"content\":\"Servo Interval Changed\"}");
  }
  else if(strcmp(SUB_TOPIC_05, topic)==0){      //  /mod/div/ser1/ir_interval
    ir_interval = (int)doc["ir_interval"];
    Div_Servo.publish(PUB_TOPIC_00, "{\"dev\":\"Div_Servo3\",\"content\":\"IR Interval Changed\"}");
  }
}
...
void pubres(const char* orderno,int flag){
  ...
  if(flag==1){
    Div_Servo.publish(PUB_TOPIC_00, "{\"dev\":\"Div_Servo3\",\"content\":\"Divide Success\"}");
  }
  else{
    Div_Servo.publish(PUB_TOPIC_00, "{\"dev\":\"Div_Servo3.\",\"content\":\"Divide Error\"}");
  }
